...
def find_main_progenitor(nodeIndex, descendantIndex, snapnum, mbranch):
    ...
    ptr = ms.match(descendantIndex, nodeIndex)
...
def find_progenitor_branch_mass_yan(nodeIndex, descendantIndex, snapnum, mass):
    """
    Identify main progenitor branch mass of each object.
    This version attempts to reproduce Yan's results.

    Returns main branch mass and main progenitor ID.
    """
    ...
    for i in range(n):
        ...
        if ptr[i] >= 0:
            if snap[ptr[i]] == snap[i] + 1: # Seems to be necessary to most closely reproduce Yan's outputs
...
def find_progenitor_tree_mass(nodeIndex, descendantIndex, snapnum, mass):
    """
    Identify main progenitor branch mass of each object.
    This version sums all progenitors.
